...
2. Create a REAL TEST WITH REAL DATA
3. For example I want to test the full flow of the allocator.
    a. get real data from repositories
    b. inject it into the allocate function
    c. the test will be graded on the output and the overall functionality
    d. once that is done u can linbt and pytest
4. Same thing with an agent test. Keep it simple, keep it real how I would actually run a test in a test file
