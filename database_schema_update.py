...
def recreate_database_schemas(output_file="database_schemas.json"):
    ...
    try:
        ...
        for db_name in equity_databases:
            ...
            # Preserve the original structure for this sector if it exists
            if db_name in original_schema:
                ...
                try:
                    ...
                    for schema_name in schemas:
                        ...
                        if schema_name in original_schema[db_name]["schemas"]:
                            # Use the original schema's table structure
                            schemas_data[db_name]["schemas"][schema_name] = {
                                "tables": {}
                            }
                            
                            # Get the original tables
                            original_tables = original_schema[db_name]["schemas"][schema_name]["tables"]
                            
                            # Process each original table to populate with current tickers
                            for table_name, table_info in original_tables.items():
                                print(f"    Processing table: {table_name}")
                                all_tickers = []  # Store all tickers from all sources
                                
                                # 0. DIRECT DATABASE QUERY - Force check the actual sector database for this table
                                # This is the most direct way to get the true data if it exists
                                try:
                                    # Always use a fresh connection for each major query to avoid transaction issues
                                    direct_conn = create_connection(db_config, db_name)
                                    direct_cursor = direct_conn.cursor()
                                    
                                    # Try to find a table directly matching the name in the sector DB
                                    direct_cursor.execute(f"""
                                    ...
                                    if direct_cursor.fetchone():
                                        ...
                                        if direct_cursor.fetchone():
                                            print(f"      Found direct table match: {schema_name}.{table_name}")
                                            direct_cursor.execute(f"""
                                                SELECT DISTINCT ticker FROM {schema_name}.{table_name}
                                            """)
                                            
                                            direct_tickers = [clean_ticker(row[0]) for row in direct_cursor.fetchall()]
                                            if direct_tickers:
                                    ...
                                    print(f"      Error with direct table query: {e}")
                                    if 'direct_conn' in locals() and direct_conn:
                                        direct_conn.close()
                                ...
                                print(f"      Final ticker count: {ticker_count} (original: {original_count})")
                            ...
                            print(f"    Schema {schema_name} not in original, skipping")
                ...
                print(f"  Database {db_name} not in original schema, skipping")
