...
# def calculate_dollar_average_volume(ticker: str, days: int = 252) -> float:
#     """
#     Calculate the dollar average volume for a given ticker over a specified number of days.
#     Dollar volume = price * volume for each trading period.
    
#     Args:
#         ticker (str): The stock ticker symbol
#         days (int): Number of days to calculate average over (default: 252)
        
#     Returns:
#         float: Dollar average volume, or 0.0 if no data found
#     """
...
#         repo = EquityPriceDataRepository()
        
#         # Calculate date range
#         end_date = datetime.now()
#         start_date = end_date - timedelta(days=days)
        
#         # Fetch price data with daily interval
#         df = repo.fetch_equity_price_data(ticker, start_date, end_date, interval='1D')
        
#         if df.empty or 'volume' not in df.columns or 'close' not in df.columns:
#             return 0.0
            
#         # Calculate dollar volume for each period (price * volume)
#         df['dollar_volume'] = df['close'] * df['volume']
        
#         # Calculate average dollar volume
#         average_dollar_volume = df['dollar_volume'].mean()
        
#         return float(average_dollar_volume) if average_dollar_volume is not None else 0.0
        
#     except Exception as e:
#         print(f"Error calculating dollar average volume for {ticker}: {e}")
#         return 0.0

# if __name__ == "__main__":
#     for ticker in full_list:
#         if calculate_dollar_average_volume(ticker) < 3_000_000: 
#             print(f"{ticker}: {calculate_dollar_average_volume(ticker)}")