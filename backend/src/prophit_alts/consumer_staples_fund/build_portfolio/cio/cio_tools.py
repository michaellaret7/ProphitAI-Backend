from backend.src.utils.validation_utils import validate_ticker, validate_portfolio_dict
...
def analyze_portfolio_performance(portfolio_dict: Any, risk_free_rate: float = 0.02) -> dict:
    ...
    try:
        ...
        if isinstance(portfolio_dict, str):
            try:
                ...
                return {"error": "Invalid portfolio_dict string. Must be valid JSON with double quotes."}
            ...
            return {"error": "portfolio_dict must be a dict or JSON string per Dictionary Format Rules"}

        # Validate portfolio structure and values
        portfolio = validate_portfolio_dict(portfolio)
    except ValueError as e:
        return {"error": str(e)}
    except Exception as e:
        return {"error": f"Error parsing portfolio: {str(e)}"}
    ...
    if not prices_map:
        return {"error": "No price data available for provided tickers"}
    ...
    if not weights:
        return {"error": "No overlapping tickers between portfolio and returns data"}
...
def get_most_recent_fundamentals(ticker: str, fundamentals_type: str) -> dict:
    ...
    # Validate fundamentals_type
    valid_types = ['balance_sheet', 'income_statement', 'cash_flow_statement', 'financial_ratios', 'analyst_estimates', 'all']
    if fundamentals_type not in valid_types:
        return {"error": f"Invalid fundamentals_type. Must be one of: {valid_types}"}
    ...
    if fundamentals_type == 'balance_sheet':
        ...
        return {"error": f"Invalid fundamentals type: {fundamentals_type}"}
