...
from prophitai_data.clients.snaptrade.auth import SnapTradeAuth
from prophitai_data.clients.snaptrade.accounts import SnapTradeAccounts
from prophitai_data.clients.snaptrade.models.activities import ActivityRecord
from prophitai_data.clients.snaptrade.models.holdings import STPortfolio
from prophitai_data.clients.snaptrade.models.orders import OrderRecord
from prophitai_data.clients.snaptrade.trading import SnapTradeTrading
from prophitai_data.clients.snaptrade.connections import SnapTradeConnections
...
class SnapTradeBroker:
    ...
    def get_orders(
        ...
        """
        Fetch orders via SnapTrade's real-time orders endpoint.

        Args:
            user_id: SnapTrade user ID
            user_secret: SnapTrade user secret
            account_id: Brokerage account ID
            state: Order state filter — 'all', 'open', or 'executed'
            days: Number of days to look back (default 30)

        Returns:
            List of OrderRecord objects
        """
        response = self.accounts._accounts.get_user_account_orders(
            user_id=user_id,
        ...
        return OrderRecord.from_raw_list(response.body)
    ...
    def get_activities(
        ...
        """
        Fetch BUY and SELL settled activities concurrently from the daily-cached ledger.
