"""
Optimized ETF data loader for adding ETF tickers to the database.
Loads comprehensive data including prices, holdings, ETF info, and dividends.
"""
...
class OptimizedETFDataLoader:
    ...
    def _get_intraday_prices_optimized(self):
        ...
        limit_date = get_current_utc_time() - timedelta(days=365 * self.years_of_history)
        call_count = 0
    ...
    def _load_price_data_optimized(self):
        ...
        start_date = get_current_utc_time() - timedelta(days=365 * self.years_of_history)
    ...
    def _load_etf_info(self):
        ...
        if etf_info_data and len(etf_info_data) > 0:
            ...
            print(f"[{self.ticker}] No ETF info data found")
    ...
    def _load_etf_holdings(self):
        ...
        if holdings_data:
            ...
            for holding in holdings_data:
                ...
                # Reason: Handle duplicates in the current batch by making asset keys unique
                ...
                holding_record = {
                    ...
                    'securityCusip': holding.get('cusip'),
                    'sharesNumber': holding.get('sharesNumber'),
                    'weightPercentage': holding.get('weightPercentage'),
                    'marketValue': holding.get('marketValue'),
                    'updatedAt': get_current_utc_time()
                }
            ...
            if duplicate_count > 0:
                print(f"[{self.ticker}] Renamed {duplicate_count} duplicate assets to unique identifiers")
            ...
            if holdings_records:
                ...
                print(f"[{self.ticker}] No valid holdings data found")
        else:
            print(f"[{self.ticker}] No holdings data found")
        ...
        try:
            ...
            if is_partial_reload:
                ...
                if not has_price_data:
                    print(f"\n[{self.ticker}] Aborting: No price data available for this ETF.")
...
def cleanup_etf_data(ticker: str):
    """
...
def load_multiple_etfs(etf_list: list, years_of_history: int = 2):
    """
    ...
    for idx, item in enumerate(etf_list):
        ...
        print(f"\n{'#'*60}")
        print(f"# Processing ETF {idx + 1}/{len(etf_list)}: {ticker}")
        print(f"{'#'*60}")
