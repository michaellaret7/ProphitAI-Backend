...
from prophitai_data.db.models.market import Ticker, Price
from prophitai_data.session.decorators import with_session, with_transaction
from prophitai_data.jobs.market import UpdatePriceTable
...
def scan_and_write_csv():
    """Scan for timezone mismatches and write results to CSV."""
    mismatched = detect_timezone_mismatches()
    write_to_csv(mismatched)
    logger.info("Scan completed successfully")


def process_all_entry():
    """Process all tickers from CSV (applies fixes)."""
    process_all_tickers(dry_run=False)
