...
from prophitai_api.schemas.portfolio import (
    PortfolioInput,
    PortfolioPosition,
    PositionType,
)
...
def require_broker_connection(clerk_id: str) -> None:
    """Raise BrokerNotConnectedError if the user has no connected brokerage account.

    Uses the lightweight DB-only connection status check (indexed on clerk_id).

    Args:
        clerk_id: Clerk authentication ID.

    Raises:
        BrokerNotConnectedError: If the user has no connected broker.
    """
