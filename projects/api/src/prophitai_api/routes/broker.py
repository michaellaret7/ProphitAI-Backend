...
from prophitai_api.controllers.broker import (
    # Account
...
async def get_orders(
    state: Optional[str] = Query("all", description="Order state: all, open, or executed"),
    days: int = Query(30, ge=1, le=90, description="Days to look back"),
    clerk_id: str = Depends(get_clerk_user_id),
):
    """Get orders via real-time SnapTrade orders endpoint."""
    return await get_orders_controller(
        clerk_id=clerk_id, state=state, days=days,
    )
