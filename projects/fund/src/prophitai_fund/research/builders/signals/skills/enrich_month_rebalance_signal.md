...
# Implementing Monthly Rebalance Bar Detection in enrich()
...
2. **Detect month boundaries** — compare month+year of each row to the previous row.
3. **cumsum() trick for grouping** — `new_month.cumsum()` creates a group key that increments at each new-month boundary.
4. **cumcount() for intra-month day ordinal** — `groupby(group_key).cumcount() + 1` gives the 1-indexed trading day within each month.
5. **Compare to offset** — `trading_day_of_month == rebalance_offset_trading_days` (default 2).
6. **Return `df.copy()`** — always copy df before adding columns to avoid mutating the caller's frame.
...
## Confirmed Patterns
- AQM52 (2026-04): Pattern works correctly on DatetimeIndex. Tested via `ruff check` + import check.
