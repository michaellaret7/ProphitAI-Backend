...
- **Universe indicators MUST use the framework cross-sectional cache.** The engine re-instantiates each indicator once per ticker — a naive universe indicator that rebuilds its full cross-sectional panel inside `calculate()` does O(n²) work (93s at n=36, ~58 min projected at n=220). Split into a cached `_compute_full_crosssectional(panel)` and a cheap per-ticker slice using `prophitai_algo_trading.indicators.{{crosssectional_cache_key, get_or_compute_crosssectional}}`. The cache key MUST include every indicator parameter. Pattern + template: `docs/algo_trading/universe_indicator_caching.md`. `DataResolver.resolve` auto-stamps shared-scope DataFrame blobs; strategy-local panel builders must call `stamp_shared_panel(panel)` manually before attach.
