...
class OptimizedTickerDataLoader:
    ...
    def _update_company_profile(self):
        ...
        if profile_data and len(profile_data) > 0:
            ...
            # Parse earnings announcement if available
            ...
            print(f"[{self.ticker}] ⚠️ No company profile data found")
    ...
    def _update_ticker_quote_data(self):
        ...
        if quote_data and len(quote_data) > 0:
            ...
            print(f"[{self.ticker}] ⚠️ No quote data found")
    ...
    def _load_daily_prices(self):
        ...
        # Get maximum history - FMP typically allows ~30 years for major stocks
        end_date = get_current_utc_time()
        start_date = datetime(1990, 1, 1, tzinfo=timezone.utc)  # Go back to 1990 for max history
    ...
    def _load_intraday_prices(self):
        ...
        try:
            ...
            if not price_data.empty:
                ...
                print(f"[{self.ticker}] ⚠️ No intraday price data found")
                return False
    ...
    def _load_balance_sheets(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No balance sheet data found")
            return
    ...
    def _load_income_statements(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No income statement data found")
            return
    ...
    def _load_cash_flow_statements(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No cash flow statement data found")
            return
    ...
    def _load_financial_ratios(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No financial ratio data found")
            return
    ...
    def _load_analyst_estimates(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No analyst estimate data found")
            return
    ...
    def _load_stock_news(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No stock news found")
            return
        ...
        for item in data:
            ...
            record = {
                ...
                'site': item.get('site'),
                'text': item.get('text'),
                'url': url[:512]  # Truncate to fit column size
            }
            records.append(record)
    ...
    def _load_press_releases(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No press releases found")
            return
    ...
    def _load_rating_scores(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No rating score data found")
            return
    ...
    def _load_analyst_recommendations(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No analyst recommendation data found")
            return
    ...
    def _load_price_target_summary(self):
        ...
        if not data or len(data) == 0:
            print(f"[{self.ticker}] ⚠️ No price target summary found")
            return
    ...
    def _load_price_target_news(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No price target news found")
            return
    ...
    def _load_stock_grade_news(self):
        ...
        if not data:
            print(f"[{self.ticker}] ⚠️ No stock grade news found")
            return
    ...
    def load_ticker_data(self, allow_partial_reload=False):
        ...
        try:
            ...
            # 2. Update company profile (always update for latest info)
...
def cleanup_ticker_data(ticker):
    """
...
def load_single_ticker(ticker, sector=None, industry=None, sub_industry=None, allow_partial_reload=False):
    """
...
def load_multiple_tickers(ticker_list):
    """
