...
def correlation_matrix(portfolio_dict: PortfolioInput | dict, lookback_days: int = 252) -> dict:
    ...
    # Use the new utility function to prepare data
    ...
    for t in tickers:
        if t in price_data and price_data[t] is not None and not price_data[t].empty:
            ...
            tr = ReturnsCalculator.total_returns(price_data[t], divs) if divs is not None else ReturnsCalculator.daily_price_returns(price_data[t])
