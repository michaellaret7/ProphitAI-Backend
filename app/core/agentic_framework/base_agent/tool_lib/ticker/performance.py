...
@with_bulk_price_data(lookback_days=252 * 3, include_dividends=False)
def get_ticker_performance_and_risk(
    ...
    *,
    ...
    """Performance and risk metrics over a fixed 3-year window.

    - Accepts a list or dict of tickers (decorator fetches close series into price_data)
    - Optionally computes total returns if include_dividends=True (fetch per ticker)
    """
    ds = DataService()
    ...
    # Normalize tickers to a list of upper-case strings
    if isinstance(tickers, dict):
        tickers_list = [str(t).upper() for t in tickers.keys()]
    else:
    ...
    for t in tickers_list:
        ...
        try:
            ...
            if close is None or close.empty:
                results[tkr] = {"error": f"no price data for {tkr}"}
                continue
            ...
            if r.empty:
                results[tkr] = {"error": f"failed to compute returns for {tkr}"}
                continue
            ...
            results[tkr] = {"error": f"failed to compute metrics for {tkr}: {e}"}
...
if __name__ == "__main__":
    # Example usage
    res = get_ticker_performance_and_risk(["AAPL"], include_dividends=True)
    print(res)
