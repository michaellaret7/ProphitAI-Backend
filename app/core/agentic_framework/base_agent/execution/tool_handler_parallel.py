...
Handles concurrent execution of multiple tool calls using ThreadPoolExecutor.
...
from app.core.agentic_framework.base_agent.execution import tool_handler
...
def _execute_tool(
    ...
    Called from ThreadPoolExecutor for parallel execution.
    ...
    """
    func = tool_handler.agent.tool_functions.get(name) # get the function from the tool registry 
    ...
    try:
        # Auto-inject _simulation_date for simulation agents. Copy arguments to avoid modifying the original arguments.
        ...
        # Execute and return the function with the args 
...
def execute_tools_parallel(
    ...
    try:
        with ThreadPoolExecutor(max_workers=len(parsed_calls)) as executor:
            # Submit all tasks, storing futures in order
            futures = []
            for tool_call, name, args, parse_error in parsed_calls:
                if parse_error:
                    # Don't submit to executor, just record None as placeholder
                    futures.append(None)
                else:
        ...
        # If ThreadPoolExecutor fails, still add tool responses to prevent malformed history
