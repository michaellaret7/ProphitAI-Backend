"""Domain memory system for agent-specific knowledge and insights."""
...
class DomainMemory:
    """Manages domain memories for specialized agent knowledge."""
    
    def __init__(self, agent_type: str, save_memory: bool = True, verbose: bool = False):
        """Initialize domain memory for a specific agent type.
        ...
        self.memory_dir = memory_base_dir / "memory_store" / "domain_memory"
    ...
    def _load_memories(self) -> None:
        """Load domain memories from disk."""
        try:
            if self.memory_path.exists():
                with open(self.memory_path, 'r', encoding='utf-8') as f:
                    ...
                    # Store agent info
                    self.memory_type = agent_data.get('type', 'Domain')
                    ...
                    if self.verbose:
                        ...
                        print(f"📚 Loaded {total_memories} domain memories for {self.agent_type} agent")

        except Exception as e:
            if self.verbose:
                print(f"⚠️ Failed to load domain memory: {e}")
            self.memories = {}
    ...
    def _save_memories(self) -> None:
        """Save domain memories to disk in unified format."""
        ...
        try:
            ...
            data = {
                'agent_memory': {
                    'type': getattr(self, 'memory_type', 'Domain'),
            ...
            if self.verbose:
                total_memories = sum(len(m) for m in self.memories.values())
                print(f"💾 Saved {total_memories} domain memories for {self.agent_type} agent")
        except Exception as e:
            if self.verbose:
