"""ChatAgent execution loop.

Stripped-down ReAct loop optimized for fast, interactive chat.
No planning phase, bounded iterations, terminates on text-only response.
"""
...
class ChatExecutionLoop:
    """Fast execution loop for chat - no planning, bounded iterations.

    Key differences from BaseAgent's ExecutionLoop:
    - No planning phase (no plan_first check)
    - No plan status injection
    - No task management system messages
    - No finalize tool detection
    - Terminates on text-only response (no tool calls)
    """
    ...
    def __init__(self, agent: 'ChatAgent'):
        ...
        Args:
            agent: Parent ChatAgent instance
        """
        self.agent = agent

    def execute(self) -> Dict[str, Any]:
        """Run mini ReAct loop until answer ready or max iterations.
        ...
        Returns:
            Dict with:
                ...
                - stop_reason: 'answer_ready' or 'max_iterations'
        """
