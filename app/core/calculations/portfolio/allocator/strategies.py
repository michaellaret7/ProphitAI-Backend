...
from app.core.calculations.portfolio.allocator.models import OptimizationStrategy
...
def run_strategy(
    ...
    **strategy_params,
    ...
    Args:
        constraint_builder: ConstraintBuilder with configured constraints
        mu: Expected returns series
        S: Covariance matrix DataFrame
        tickers: Ordered list of ticker symbols
        strategy: Strategy name (e.g., "max_sharpe", "min_vol")
        risk_free_rate: Risk-free rate for performance calculation
    ...
    # Only pass risk_free_rate for max_sharpe (matches original behavior)
